...
def get_dataloader(loader_type, batch_size, cache_path=DATA_CACHE_PATH):
    if Path(TOKENIZER_PATH / 'tokenizer.json').is_file():
        dataset = get_dataset(split=loader_type, cache_path=cache_path)
        flat_dataset = dataset.flatten()
        datapipe = IterableWrapper(zip(flat_dataset['translation.en'],
                                       flat_dataset['translation.de']))
        batch_dp = datapipe.bucketbatch(batch_size=batch_size,
                                        drop_last=False,
                                        batch_num=100,
                                        bucket_num=100,
                                        sort_key=sort_key)
